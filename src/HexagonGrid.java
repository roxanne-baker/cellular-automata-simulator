...
public class HexagonGrid extends Grid {
	...
	public void setGrid(int height, int width) {
		...
		double cellWidth = gridSideLength/width;
		double cellHeight = gridSideLength/(height/0.75);
		
		double midHeight = cellHeight*0.25;
		double sideLength = cellHeight*0.5;
	...
	public Double[] getNextRowStartingCoords(double cellWidth, double cellHeight, int rowInt) {
		...
		if (row % 2 == 0) {
			...
			coords[2] += cellWidth;
			coords[4] += cellWidth*(3/2);
			coords[5] += cellWidth*(3/2);
			coords[8] += cellWidth;
			coords[10] += cellWidth/2;			
		}
		
		return new Double[]{coords[0], coords[1]+cellHeight*0.25,
